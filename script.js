...
function startAudioEngine() {
  ...
  droneOscs = DRONE_FREQS.map(freq => {
    ...
    g.gain.value = freq < 50 ? 0.18 : 0.10;
    osc.connect(g);
...
function setAmbientSection(idx) {
  ...
  const configs = [
    { freqs: [40, 40.3, 80.1],   filter: 280, lfo: 0.07, gain: 0.07 }, // Earth — heavy dread
    { freqs: [55, 55.4, 110.2],  filter: 380, lfo: 0.12, gain: 0.06 }, // Launch — rising
    { freqs: [32, 32.2, 64.1],   filter: 180, lfo: 0.04, gain: 0.05 }, // Space — calm, lonely
    { freqs: [50, 75, 100],      filter: 600, lfo: 0.22, gain: 0.08 }, // Entry — tense
    { freqs: [48, 72, 96],       filter: 320, lfo: 0.06, gain: 0.05 }, // Colony — resolved
  ];
...
let lastSectionGain = 0.07; // tracks the intended gain for current section
