...
function startAudioEngine() {
  ...
  // ── Drone: 3 detuned oscillators for rich texture
  const DRONE_FREQS = [40, 40.3, 80.1]; // sub-bass + slight detuning
  droneOscs = DRONE_FREQS.map(freq => {
    const osc = audioCtx.createOscillator();
    const g   = audioCtx.createGain();
    osc.type = 'sawtooth';
    osc.frequency.value = freq;
    g.gain.value = freq < 50 ? 0.35 : 0.18;
    osc.connect(g);
  ...
  droneOscs.forEach(o => o.disconnect());
  droneOscs.forEach((o, i) => {
    const g = audioCtx.createGain();
    g.gain.value = i === 0 ? 0.35 : 0.18;
    o.connect(g);
    g.connect(droneFilter);
  });
  droneFilter.connect(masterGain);

  // ── LFO wobble (0.07 Hz — very slow breathing)
  ...
  // ── Subtle noise layer for "vacuum hiss"
...
function playLaunchRumble() {
  ...
  g.gain.linearRampToValueAtTime(0.38, audioCtx.currentTime + 0.5);
  g.gain.linearRampToValueAtTime(0.28, audioCtx.currentTime + 2.5);
...
function setAmbientSection(idx) {
  if (!audioCtx || !droneOscs.length) return;
  const configs = [
    { freqs: [40, 40.3, 80.1],   filter: 280, lfo: 0.07, gain: 0.18 }, // Earth — heavy dread
    { freqs: [55, 55.4, 110.2],  filter: 380, lfo: 0.12, gain: 0.15 }, // Launch — rising
    { freqs: [32, 32.2, 64.1],   filter: 180, lfo: 0.04, gain: 0.14 }, // Space — calm, lonely
    { freqs: [50, 75, 100],      filter: 600, lfo: 0.22, gain: 0.20 }, // Entry — tense
    { freqs: [48, 72, 96],       filter: 320, lfo: 0.06, gain: 0.12 }, // Colony — resolved
  ];
  const c = configs[idx] || configs[0];
  const t = audioCtx.currentTime;
  droneOscs.forEach((o, i) => {
    o.frequency.linearRampToValueAtTime(c.freqs[i] || c.freqs[0], t + 2.5);
  });
  if (droneFilter) {
    droneFilter.frequency.linearRampToValueAtTime(c.filter, t + 2.5);
  }
  if (droneLFO) {
    droneLFO.frequency.linearRampToValueAtTime(c.lfo, t + 2.5);
  }
...
let lastSectionGain = 0.18; // tracks the intended gain for current section
